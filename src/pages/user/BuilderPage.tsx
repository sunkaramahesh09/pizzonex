...
export default function BuilderPage() {
  ...
  const handleProceed = () => {
    if (!selections.base.length || !selections.sauce.length || !selections.cheese.length) {
  ...
  return (
    <div className="container py-8">
      <h1 className="font-display text-3xl font-bold mb-8">Build Your Pizza</h1>
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            {steps.map((s, i) => (
              <Button
                key={s}
                variant={step === i ? "default" : selections[s.toLowerCase()]?.length ? "secondary" : "outline"}
                size="sm"
                onClick={() => setStep(i)}
              >
                {selections[s.toLowerCase()]?.length ? (
                ...
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {currentOptions.map((option) => {
                      ...
                      return (
                        ...
                        >
                          {option}
                          {selected && <Check className="inline ml-2 h-4 w-4" />}
                        </button>
                      );
                    })}
                  </div>
                  <div className="flex justify-between mt-6">
                    ...
                    {step < steps.length - 1 ? (
                      ...
                      <Button onClick={handleProceed}>Add to Cart</Button>
                    )}
                  </div>
                </CardContent>
              </Card>
          ...
          <Card className="sticky top-20">
            ...
            <CardContent className="space-y-4">
              {allSelections.length === 0 ? (
                <p className="text-sm text-muted-foreground">Start selecting ingredients…</p>
              ) : (
